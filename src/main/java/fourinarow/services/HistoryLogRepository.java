...
//	@Query(value="SELECT h1.log_id, h1.game_id, (CASE WHEN h1.from_ai THEN REPLACE(REPLACE(REPLACE(h1.current_state, \"2\", \"3\"), \"1\", \"2\"), \"3\", \"1\") ELSE h1.current_state END) as current_state, h1.chosen_move, h1.won_game, h1.moves_before_end, (SELECT (CASE WHEN h2.from_ai=h1.from_ai THEN 1 ELSE 0 END) FROM history h2 WHERE h2.game_id = h1.game_id AND won_game = 1) as led_to_win FROM `history` h1 WHERE game_type = \"TTT\"", nativeQuery = true)
	@Query(value="SELECT h1.log_id as logId, h1.game_id as gameId, (CASE WHEN h1.from_ai=false THEN REPLACE(REPLACE(REPLACE(h1.current_state, \"2\", \"3\"), \"1\", \"2\"), \"3\", \"1\") ELSE h1.current_state END) as currentState, h1.chosen_move as chosenMove, h1.won_game as wonGame, h1.moves_before_end as movesBeforeEnd, coalesce((SELECT (CASE WHEN h2.from_ai=h1.from_ai THEN TRUE ELSE FALSE END) FROM history h2 WHERE h2.game_id = h1.game_id AND won_game = 1), FALSE) as ledToWin FROM `history` h1 WHERE game_type = \"TTT\"", nativeQuery = true)
